...
while webcam.isOpened():
    ...
    try:
        ...
        # find the area of max contour and hull
        max_contour_area = cv2.contourArea(max_contour)
        hull_area = cv2.contourArea(hull)

        max_contour_area_pct = ((hull_area / max_contour_area) - 1) * 100
        print(max_contour_area_pct)
        ...
        if max_contour_area_pct <= 20:
            cv2.putText(frame, f"Fingers count: 0", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)
        else:
            cv2.putText(frame, f"Fingers count: {defects_count+1}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)
